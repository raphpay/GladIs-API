...
extension UserControllerTests {
    func testAddModuleSucceed() async throws {
        ...
        let path = "\(baseRoute)/\(userID)/modules/\(moduleID)"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
            XCTAssertEqual(res.status, .ok)
            let createdModule = try res.content.decode(Module.self)
            XCTAssertEqual(createdModule.name, expectedModuleName)
            XCTAssertEqual(createdModule.index, expectedModuleIndex)
        }
    }
    
    func testAddModuleWithInexistantUserFails() async throws {
        let admin = try await User.create(username: expectedAdminUsername, on: app.db)
        let token = try await Token.create(for: admin, on: app.db)
        let module = try await Module.create(name: expectedModuleName, index: expectedModuleIndex, on: app.db)
        
        let moduleID = try module.requireID()
        let path = "\(baseRoute)/12345/modules/\(moduleID)"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
    ...
    func testAddModuleWithInexistantModuleFails() async throws {
        let user = try await User.create(username: expectedUsername, on: app.db)
        let token = try await Token.create(for: user, on: app.db)
        
        let userID = try user.requireID()
        let path = "\(baseRoute)/\(userID)/modules/12345"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
            XCTAssertEqual(res.status, .notFound)
...
extension UserControllerTests {
    func testRemoveModuleSucceed() async throws {
        ...
        let userID = try user.requireID()
        let moduleID = try module.requireID()
        let path = "\(baseRoute)/\(userID)/remove/modules/\(moduleID)"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
            XCTAssertEqual(res.status, .ok)
    ...
    func testRemoveModuleWithInexistantUserFails() async throws {
        ...
        let moduleID = try module.requireID()
        let path = "\(baseRoute)/12345/remove/modules/\(moduleID)"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
            XCTAssertEqual(res.status, .notFound)
    ...
    func testRemoveModuleWithInexistantModuleFails() async throws {
        ...
        let userID = try user.requireID()
        let path = "\(baseRoute)/\(userID)/remove/modules/12345"
        try app.test(.POST, path) { req in
            req.headers.bearerAuthorization = BearerAuthorization(token: token.value)
        } afterResponse: { res in
            XCTAssertEqual(res.status, .notFound)
